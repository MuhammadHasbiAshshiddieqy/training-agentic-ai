...
Catatan SDK: memakai `google-genai` (paket resmi terbaru). Model embedding
`gemini-embedding-001` menghasilkan vektor 3072 dimensi secara default;
di sini kita minta 768 dimensi lewat `output_dimensionality` supaya
konsisten dengan skema tabel `documents` (VECTOR(768)).
"""
import glob
import os
import time
...
def embed_text(text: str, retries: int = 3) -> list[float]:
    ...
    (model gemini-embedding-001, dipangkas ke 768 dimensi).
    """
