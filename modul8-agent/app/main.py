"""
Modul 8 - FastAPI App (lanjutan Modul 7) + Agent Foundation (LENGKAP)
AI Knowledge Assistant - Human Initiative

Menambahkan endpoint /agent yang memakai loop Think-Act-Observe (lihat
...
async def chat(payload: ChatMessage, api_key: str = Depends(verify_api_key)):
    ...
    if response.function_calls:
        ...
        for fc in response.function_calls:
            ...
            # dengan function_calls sebelumnya.
        ...
        # /agent di bawah untuk loop multi-langkah), jadi di sini kita beri
        # pesan yang jelas alih-alih membiarkan ChatResponse(answer=None) crash 500.
...
async def run_agent(payload: AgentGoal, api_key: str = Depends(verify_api_key)):
    ...
    satu permintaan, memakai hasil satu tool untuk memutuskan tool
    berikutnya. Bandingkan dengan /chat (Modul 7) yang hanya satu putaran.

    Contoh goal yang menunjukkan bedanya:
      "Cek stok genset. Kalau kurang dari 5 unit, cari SOP permintaan
      barang tambahan untuk logistik darurat."
    """
    if not GEMINI_API_KEY:
        raise HTTPException(status_code=500, detail="GEMINI_API_KEY belum di-set")
    ...
    try:
        # Agent.run melakukan panggilan sinkron ke Gemini; dijalankan di
        # thread terpisah supaya tidak memblokir event loop FastAPI.
        import asyncio
