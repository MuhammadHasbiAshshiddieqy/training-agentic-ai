"""
Modul 11 - FastAPI App (lanjutan Modul 9) + Observability & Evaluation (LENGKAP)
AI Knowledge Assistant - Human Initiative

Menambahkan instrumentasi Langfuse (lihat observability.py) di seluruh
endpoint yang memanggil Gemini atau pipeline retrieval, plus mencatat
event cache hit/miss dari Modul 9 sebagai metadata trace. Modul 10
(Guardrails) sengaja dilewati - modul ini dibawa maju langsung dari
Modul 9.

Skrip evaluasi otomatis ada di evaluate.py (dijalankan terpisah lewat
`docker compose exec app python evaluate.py`, BUKAN endpoint HTTP) -
menembak endpoint /chat berkali-kali dengan set pertanyaan tetap, menilai
jawabannya (Gemini-as-judge), dan mencatat skornya sebagai score di
Langfuse supaya trend kualitas terlihat dari waktu ke waktu.
...
from observability import observe, get_client, log_gemini_usage, log_cache_event, LANGFUSE_ENABLED

app = FastAPI(title="AI Knowledge Assistant - Modul 11", version="11.0.0")
...
async def health_check():
    return {"status": "ok", "observability_enabled": LANGFUSE_ENABLED}
...
@app.get("/gemini-test")
@observe(name="gemini-test-endpoint")
async def gemini_test():
    """Panggilan Gemini paling sederhana di kit ini - contoh trace paling kecil untuk mulai belajar dashboard Langfuse."""
    ...
    try:
        ...
        log_gemini_usage(GEMINI_MODEL, response)
...
# ---------------------------------------------------------------------------
# MODUL 5/6/11 - Production RAG + Hybrid Search, sekarang ter-trace penuh
# lewat @observe di retrieval.py (tiap tahap = satu observation anak).
# ---------------------------------------------------------------------------
...
async def search(
    ...
    """
    Hybrid Search RAG - empat tahap (lihat retrieval.py, masing-masing
    sudah jadi observation tersendiri di Langfuse: vector_search,
    keyword_search, rrf_merge, rerank_candidates, assemble_context).
    """
    try:
...
# ---------------------------------------------------------------------------
# MODUL 7/9/11 - Tool Calling + Semantic Cache + Observability
# ---------------------------------------------------------------------------
...
@observe(as_type="generation", name="chat-generate")
def _generate_chat_response(client, contents, config, phase: str):
    """Satu panggilan Gemini generate_content di dalam /chat, dipisah jadi fungsi sendiri supaya jadi satu observation "generation" per putaran (Gemini 3.x kadang butuh 2 putaran: jawaban awal + jawaban setelah hasil tool dikirim balik)."""
    response = client.models.generate_content(model=GEMINI_MODEL, contents=contents, config=config)
    log_gemini_usage(GEMINI_MODEL, response, extra_metadata={"phase": phase})
    return response
...
async def chat(payload: ChatMessage, api_key: str = Depends(verify_api_key)):
    """
    Endpoint chat dengan tool calling + semantic cache + tracing penuh.
    Alur (tidak berubah dari Modul 9, hanya ditambah observability):
      1. Embed pertanyaan user, cek semantic cache (Modul 9). Event
         hit/miss dicatat ke trace Langfuse lewat log_cache_event().
      2. MISS -> jalankan tool calling seperti Modul 7 lewat
         _generate_chat_response() (jadi observation "generation" di
         Langfuse, lengkap dengan token usage).
      3. Simpan jawaban baru ke cache KECUALI melibatkan REALTIME_TOOLS.
    """
    if not GEMINI_API_KEY:
    ...
    if hit is not None:
        log_cache_event(layer="semantic", hit=True, similarity=hit["similarity"])
        elapsed_ms = int((time.perf_counter() - start) * 1000)
        return ChatResponse(
            answer=hit["answer"],
    ...
    log_cache_event(layer="semantic", hit=False)
    ...
    try:
        response = _generate_chat_response(client, contents, config, phase="initial")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Gagal memanggil Gemini API: {e}")
    ...
    if response.function_calls:
        ...
        try:
            final_response = _generate_chat_response(client, contents, config, phase="after_tool_call")
        except Exception as e:
    ...
    elapsed_ms = int((time.perf_counter() - start) * 1000)
    get_client().update_current_span(output={"cacheable": cacheable, "tools_called": [t.name for t in tools_called]})
    return ChatResponse(
        answer=answer_text,
...
# ---------------------------------------------------------------------------
# MODUL 8/11 - Agent Foundation (loop Think-Act-Observe), sekarang ter-trace
# penuh lewat @observe di agent.py (tiap iterasi THINK = generation anak,
# tiap ACT otomatis dapat anak-span dari tool yang dipanggil).
# ---------------------------------------------------------------------------
...
async def run_agent(payload: AgentGoal, api_key: str = Depends(verify_api_key)):
    ...
    satu permintaan. Seluruh perjalanan think/act/observe-nya (lihat
    agent.py) muncul sebagai satu trace bersarang penuh di Langfuse.
    """
    if not GEMINI_API_KEY:
        raise HTTPException(status_code=500, detail="GEMINI_API_KEY belum di-set")
