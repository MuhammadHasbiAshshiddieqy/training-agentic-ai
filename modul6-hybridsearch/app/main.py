"""
Modul 6 - FastAPI App (lanjutan Modul 5) + Hybrid Search (LENGKAP)
...
algoritma Reciprocal Rank Fusion (RRF), SEBELUM tahap rerank & context
assembly dari Modul 5 tetap dijalankan seperti biasa.
...
app = FastAPI(title="AI Knowledge Assistant - Modul 6", version="6.0.0")
...
RRF_K = 60  # konstanta RRF standar dari literatur (Cormack, Clarke & Büttcher, 2009)
...
def vector_search(conn, query_embedding, top_k: int, category: str | None = None) -> list[dict]:
...
def keyword_search(conn, query: str, top_k: int, category: str | None = None) -> list[dict]:
    """
...
async def search(query: str, limit: int = 3, category: str | None = None):
    ...
    Hybrid Search RAG — empat tahap:
      1. Retrieval GANDA: vector search DAN keyword search berjalan
         masing-masing, ambil RETRIEVE_TOP_K kandidat
      2. RRF merge: gabungkan kedua ranked list jadi satu daftar kandidat
         berdasarkan Reciprocal Rank Fusion
      3. Rerank: Gemini menilai ulang kandidat gabungan (Modul 5), ambil
         `limit` terbaik
      4. Context assembly: gabungkan jadi satu context siap pakai (Modul 5)
    """
