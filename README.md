...
| 9 | Diuji **live** dengan API key Gemini asli: `/chat` cache MISS→HIT, `cacheable: false` untuk tool real-time, `/search` dengan embedding cache, `/cache-stats` | Cache MISS `latency_ms: 11527` → cache HIT `latency_ms: 484` (24x lebih cepat, `cache_similarity: 0.982`); jawaban `cek_stok_barang` terbukti `cacheable: false`, tidak pernah masuk semantic cache |
